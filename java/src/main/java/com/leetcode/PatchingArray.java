...
public class PatchingArray {
    public int minPatches(int[] nums, int n) {
        ...
        buildSum(nums, 0, 0, sums);

        // find all missing numbers
        List<Integer> missing = new ArrayList<>();
        for (int i = n; i >= 1; --i) {
            if (!sums.contains(i)) missing.add(i);
        }
    ...
    // @param i: ith number in the array nums
    // @param sum: current sum
    private void buildSum(int[] nums, int i, int sum, Set<Integer> sums) {
        if (i == nums.length) {
            sums.add(sum);
        } else {
            buildSum(nums, i + 1, sum, sums);
            buildSum(nums, i + 1, sum + nums[i], sums);
        }
    }
}
